...
static float rescale(float t, float min, float max, PlotConfig::Scale::Type type)
{
    ...
    case PlotConfig::Scale::Log10:
        return std::log10(ImLerp(min, max, t) / min) / std::log10(max / min);
    }
    return 0;
}
...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    if (conf.values.count > 0) {
        ...
        auto x_min = static_cast<float>(conf.values.offset);
        auto x_max = static_cast<float>(conf.values.offset + conf.values.count - 1);
        if (conf.values.xs) {
        ...
        if (conf.grid_x.show) {
            ...
            case PlotConfig::Scale::Linear: {
	            auto cnt = static_cast<float>(conf.values.count) / (conf.grid_x.size / static_cast<float>(conf.grid_x.subticks));
                float inc = 1.f / cnt;
                for (int i = 0; i <= cnt; ++i) {
        ...
        for (int i = 0; i < ys_count; ++i) {
            ...
            for (int n = 0; n < res_w; n++)
            {
                ...
                const int v1_idx = static_cast<int>(t0 * static_cast<float>(item_count) + 0.5f);
        ...
        if (conf.selection.show) {
            ...
            float fSelectionStep = 1.0f / static_cast<float>(item_count);
